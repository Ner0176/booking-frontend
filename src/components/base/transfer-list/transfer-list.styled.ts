...
export const TFColumnWrapper = styled.div(
  tw`flex flex-col gap-3 px-2 w-full max-h-[275px] overflow-y-auto`
);
export const TFItemContainer = styled.div<{ isSelected?: boolean }>`
  ${tw`text-sm sm:text-base cursor-pointer rounded-lg px-4 py-2 border text-center shadow-sm`}
  ${({ isSelected }) =>
    isSelected
